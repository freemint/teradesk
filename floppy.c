...
static void fpartoftext(_WORD tsides,	/* number of sides */
...
static void prdisp(_WORD current,		/* current track, =-1 for 0% */
...
static long onetrack(_WORD what,		/* 8 = read, 9= write */
...
static bool checktracks(_WORD tsides,	/* Number of sides */
						_WORD ttracks,	/* Number of tracks */
...
{
	if ((tsides < 1) || (tsides > 2) ||	/* number of sides: 1 or 2 */
...
static _WORD readfp(_WORD dev, unsigned char *sect0, _WORD *bps, _WORD *spt, _WORD *sides, _WORD *tracks, _WORD *dirsize)
{
	_WORD istat;
	_WORD secs;
...
void formatfloppy(char fdrive,			/* drive id letter ( 'A'or 'B' ) */
				  _WORD format			/* true for format, false for copy */
	)
{
	unsigned char fc;
	unsigned char *sect0;
	unsigned char *label;				/* pointer to a position within sect0 */

	long filler = 0;					/* unused but required filler */
	long serial;							/* 24-bit disk serial number  */
	long istat;							/* function execution status  */
#if _MINT_
	long lstats = 0;					/* same, for locking source device */
	long lstatt = 0;					/* same, for locking target device */
#endif
	long mbsize = 0;					/* allocated memory block size (bytes) */

	_WORD button;						/* index of activeded button */
	_WORD sectors = 0;					/* total number of disk sectors */
	_WORD itrack;						/* current track (counter) */
	_WORD ftracks;						/* how many tracks to format */
	_WORD iside;						/* current side (counter) */
	_WORD sbps;							/* source disk bytes per sector */
	_WORD tbps;							/* target disk bytes per sector */
	_WORD sdevno = 0;					/* source drive id */
	_WORD tdevno;						/* target disk drive id */
	_WORD ssides;						/* source disk number of sides */
	_WORD stracks;						/* source disk number of tracks */
	_WORD sspt;							/* source disk number of sectors per track */
	_WORD ipass;						/* copy pass counter */
	_WORD npass;						/* number of copy passes */
	_WORD finished;						/* =1 if successfully finished operation */
	_WORD tpp;							/* tracks per pass  */
	_WORD i, j, n;						/* aux. counters, etc. */
	_WORD retrycnt;						/* limit for retry counter */
	_WORD rc;								/* retry counter */

	static _WORD mspt = 11;				/* maximum permitted sectors per track */
	static _WORD tsides;				/* target disk number of sides */
	static _WORD ttracks;				/* target disk number of tracks */
	static _WORD tspt;					/* target disk number of sectors per track */
	static _WORD intleave = 1;			/* sector interleave */
	static _WORD fatsize = 3;			/* FAT size (sectors), take other initial values from dialog */
	static _WORD dirsize;				/* number of directory entries */

	char drive[4];						/* string with drive id */
	char fmtsel;						/* flag for selected format type */

	static unsigned char fat0 = 0xf9;	/* first byte of each FAT */

	static const unsigned char hdr[] =	/* MS-DOS compatible header of the boot sector */
	{
		0xeb,							/*  [0] */
	  ...
	  again:							/* return here if invalid format parameters */
		...
		while (button != FMTOK && button != FMTCANC)
		{
			...
			if (format)
			{
				...
				case FPREV:			/* Previous disk format */
					fmtsel = 0;
					hourglass_mouse();
					if (readfp(tdevno, sect0, &tbps, &tspt, &tsides, &ttracks, &dirsize) == 0)
					{
						fat0 = sect0[0x15];
						fatsize = sect0[0x16];
						fmtsel = 2;
						...
						strncpy
							(fmtfloppy[FLABEL].ob_spec.tedinfo->te_ptext,
							 (char *) (&sect0[(2 * fatsize + 1) * tbps]), 11);
					}
					arrow_mouse();
					break;
		...
		if (button == FMTOK)
		{
			...
			if (format)
			{
				...
				if (!checktracks(tsides, ttracks, tspt, mspt, tbps) || (fatsize < 1) || (fatsize > ((tspt - 1) / 2)) ||	/* two fats + sector 0 fit on a track */
			...
			if (button == 1)
			{
					...
					for (itrack = 0; itrack < ftracks; itrack++)
					{
						for (iside = 0; iside < tsides; iside++)
						{
						  ...
						  retryf:		/* come here for a retry after formatting error */
							...
							istat =
								Flopfmt(sect0, (void *) filler, tdevno, tspt, itrack, iside, intleave, 0x87654321L,
										0xE5E5);
				...
				} else					/* disk copy */
				{
					...
					istat = readfp(tdevno, sect0, &tbps, &tspt, &tsides, &ttracks, &dirsize);
					...
					istat = readfp(sdevno, sect0, &sbps, &sspt, &ssides, &stracks, &dirsize);
					...
					i = 0;
					j = 0;				/* this will now be the source and target track counters */
