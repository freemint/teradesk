...
/* 
 * pref TT-ram (base->mode) default 0 for ST,
 * which is nice because we leave the working to Malloc itself
 * if TT ram exists. 
 * Note: XA_memory = {*first, *last, *cur, chunk, head, mode}
 */

XA_memory XA_default_base = {NULL, NULL, NULL, 16384, 16, 0};

static void *new_block(XA_memory *base, long size, bool svcur)
{
...
static void free_block(XA_memory *base, XA_block *blk)
{
...
static void free_unit(XA_list *list, XA_unit *this)
{
	XA_unit 
		*next = this->next, 
		*prior = this->prior;
...
static XA_block *find_free(XA_memory *base, long s)
{
	XA_block *blk;
...
static XA_unit *split(XA_list *list, long s)
{
...
static void insfirst(XA_list *list, XA_unit *this)
{
	DBG(("insfirst %ld :: %ld\n", this, this->size);)
...
static void insbefore(XA_list *list, XA_unit *this, XA_unit *before)
{
	DBG(("insbefore %ld\n", before);)
...
static void inslast(XA_list *list, XA_unit *this)
{
	DBG(("inslast %ld :: %ld\n", this, this->size);)
...
static void insafter(XA_list *list, XA_unit *this, XA_unit *after)
{
	DBG(("insafter %ld\n", after);)
...
static void amalgam(XA_list *list, XA_unit *this)
{
	if (this->next)
	{
...
static void sortins(XA_list *list, XA_unit *this)
{
	XA_unit *have;
...
static void combine(XA_list *list, XA_unit *this)
{
	DBG((" - a1\n");)
...
static void free_area(XA_memory *base, XA_block *this, XA_unit *at)
{
DBG((" - free_unit\n");)
...
/* ATTENTION: is this correct? perhaps the parentheses should be here! */
...
/* 
 * Free a memory block;
 * Note: if area=NULL, nothing happens
 */
