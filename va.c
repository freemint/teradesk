...
void handle_av_protocol(const _WORD *message)
{
			...
			} else
			{
				...
				if ((aw = xw_find(m3, message[4])) != NULL)
				{
					...
					} else if ((item = itm_find(aw, m3, message[4])) >= 0)
					{
						...
						if ((int)itype >= ITM_NOTUSED && itype <= ITM_NETOB)
						{
	...
	case AV_FILEINFO:
	  ...
	  processname:;
		...
		if (path)
		{
			char *p = path,			/* current position in the string */
				*wpath = NULL,		/* simulated window path */
				*cs,
				*cq,				/* position of the next " " and "'" */
				*pp = NULL;			/* position of the next name */

			bool q = FALSE;			/* true if name is quoted */

			ITMTYPE itype;			/* type of the item */

			DIR_WINDOW ww;			/* structure for the simulated window */

			_WORD list = 0;			/* simulated selected item */

			*global_memory = 0;		/* clear the string in the buffer */
			status = TRUE;			/* all is well for the time being */
			...
			while (status && p && *p)
			{
				...
				if (itype != ITM_NOTUSED && itype != ITM_NETOB)
				{
					...
					{
					case AV_DRAG_ON_WINDOW:
						status = itm_move((WINDOW *) & ww, 0, m3, message[4], message[5]);
						break;
					case AV_COPYFILE:
						{
							/* 
							...
							status = mask ? itmlist_op((WINDOW *) & ww, 1, &list, mask, (message[7] & 0x0001) ? CMD_MOVE : CMD_COPY) : 0;
							options.cprefs = old_prefs;
						}
						break;
					case AV_DELFILE:
						status = itmlist_wop((WINDOW *) & ww, 1, &list, CMD_DELETE);
						break;
					case AV_FILEINFO:
						*(char **) (answer + 6) = global_memory;
						item_showinfo((WINDOW *) & ww, 1, &list, FALSE);
						status = TRUE;	/* but it is not always so! */
						break;
					}
	...
	case FONT_SELECT:
		...
		fnt_mdialog(message[1], m3, message[4], message[5], message[6], message[7], 1);
		reply = FALSE;
		break;
#endif
	...
	} else
	{
		...
		*error = CfgLoad(file, stat_table, MAX_CFGLINE, lvl);
		...
		if (*error == 0)
		{
			...
			{
				char *statfile = malloc(strlen(this.stat) + 1);

				if (statfile)
				{
					strcpy(statfile, this.stat);
					strcpy(avswork.name, this.name);
					avswork.stat = statfile;

					if (lsadd_end((LSTYPE **) & avstatus, sizeof(avswork), (LSTYPE *) & avswork, copy_avstat) == NULL)
					{
						free(statfile);
						*error = ENOMSG;
					}
