...
#else
#define DEFAULT_EXT		"*.*"
#define ITEMLEN			51				/* Length of a line. */
#endif
...
{
	if (w->flags.fulled == 1)
		dir_wsize(w->dir_window, (GRECT *) &screen_info.dsk_x, size);
	else
	{
		GRECT def, border;
						 ...
						 GRECT *work)
{
	...
	if ((obj = malloc((n + 1) * sizeof(OBJECT) + n * sizeof(ICONBLK))) == NULL)
	{
		xform_error(ENSMEM);
		return NULL;
	}

	icnblk = (ICONBLK *) & obj[n + 1];
	...
	for (i = 0; i < n; i++)
	{
		...
		icnblk[i] = icons[icon_no];
		icnblk[i].ib_ptext = (char *) h->name;
		icnblk[i].ib_char &= 0xFF00;
		icnblk[i].ib_char |= 0x20;
...
static void icn_comparea(DIR_WINDOW *dw, int item, GRECT *r1, GRECT *r2, GRECT *work)
{
	...
	h = &icons[dw->buffer[(long) item].icon];
...
boolean dir_add_window(const char *path)
{
	...
	{
		if ((fspec = strdup(DEFAULT_EXT)) == NULL)
		{
			xform_error(ENSMEM);
			free(path);
			return FALSE;
