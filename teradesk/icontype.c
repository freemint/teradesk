...
static void set_selector(SLIDER *slider, boolean draw, XDINFO *info)
{
	...
	addicntype[AITDATA].ob_x = (addicntype[AITBACK].ob_width - h2->ib_wicon) / 2;
	addicntype[AITDATA].ob_y = (addicntype[AITBACK].ob_height - h2->ib_hicon) / 2;
...
static int load_list(XFILE *file, ICONTYPE **list)
{
	...
	do
	{
		...
		if (it.icon != -1)
		{
			...
			if (add(list, name, it.icon, END) == NULL)
				return ERROR;
		}
	}
	while (it.icon != -1);
