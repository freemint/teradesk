...
{
	fileinfo[ISWP].ob_state = (attribs & FA_READONLY) ? SELECTED : NORMAL;
	fileinfo[ISHIDDEN].ob_state = (attribs & FA_HIDDEN) ? SELECTED : NORMAL;
	fileinfo[ISSYSTEM].ob_state = (attribs & FA_SYSTEM) ? SELECTED : NORMAL;
}
...
static int get_file_attribs(int old_attribs)
{
	...
	attribs |= (fileinfo[ISWP].ob_state == SELECTED) ? FA_READONLY : 0;
